...
export default function ChatInput({ onSend, theme, text, setText, replyingTo, cancelReply, smartReplies }: ChatInputProps) {
  ...
  return (
    <div className="px-4 py-4 relative border-t" style={{ backgroundColor: theme.header, borderColor: theme.border }}>
      ...
      {/* Emoji Picker */}
      {showEmojis && (
         <div className="absolute bottom-20 left-4 bg-white shadow-xl rounded-2xl border w-64 h-64 overflow-y-auto grid grid-cols-6 p-2 z-50 animate-in zoom-in-95" style={{ borderColor: theme.border }}>
            {EMOJIS.map(e => (
                <button key={e} onClick={() => handleEmojiClick(e)} className="p-2 hover:bg-gray-100 rounded text-xl transition-colors">
                    {e}
                </button>
            ))}
         </div>
      ...
      <div className="flex items-center space-x-2">
        <button 
            ...
            className={`p-2 rounded-full transition-colors ${showEmojis ? 'bg-indigo-100 text-indigo-600' : 'text-gray-500 hover:bg-gray-100'}`}
            title="Emojis (Ctrl+E)"
        >
            <Smile size={24} />
        </button>
        ...
        {text ? (
            <button onClick={() => onSend(text)} className="p-2 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 transition-colors transform active:scale-90">
                <Send size={20} />
            </button>
        ) : (
            <button className="p-2 text-gray-500 hover:bg-gray-100 rounded-full transition-colors">
